...
int bfs(int **graph, int **book, int n, int m, int source_x, int source_y, int dest_x, int dest_y){
	...
	tail++;
	while(head<tail){
		//统计在队列头部的点可以一次性消灭的敌人数目
		if(get_sum(graph, queue[head].x, queue[head].y) > max){
			max = get_sum(graph, queue[head].x, queue[head].y);
		}
		//共有上下左右四个方向
		for(i=0;i<4;i++){
...
//统计上下左右总共可以消灭的敌人数目
int get_sum(int **graph, int source_x, int source_y){
	int x, y, sum = 0;
...
int main()
{
	...
	// for test, (0, 0)-->(3, 2)
	book[0][0] = 1;
	max = bfs(graph, book, n, m, 0, 0, 1, 2);
	printf("max enemies are %d\n", max);
